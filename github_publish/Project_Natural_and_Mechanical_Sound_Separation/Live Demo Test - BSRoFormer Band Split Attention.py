# ==============================================================================
# 🎧 LIVE DEMO TEST — PRETRAINED BS-ROFORMER / BAND-SPLIT ATTENTION MODEL
# 📌 Project: Natural and Mechanical Sound Separation (NatSep)
# 📄 Description: Real-time inference test with official Pretrained Band-Split Transformer model.
...
# 📌 2. OFFICIAL PRETRAINED BAND-SPLIT MODEL NAME
# Options: 'htdemucs_ft' (Meta's best pretrained fine-tuned Band-Split Transformer), 'htdemucs'
...
def main():
    ...
    # Load Official Pretrained BS-RoFormer / Band-Split Transformer Model
    print(f"⏳ Downloading & Loading Official Pretrained Band-Split SOTA Model '{MODEL_NAME}'...")
    try:
    ...
    model.to(device)
    model.eval()
    print(f"✅ Official Pretrained Band-Split SOTA Model Loaded! Model sources: {model.sources}")
    ...
    if audio.shape[-1] > int(PREVIEW_SECONDS * sr):
        orig_preview = f"preview_original_bsroformer_{os.path.basename(INPUT_AUDIO_PATH)}.wav"
    ...
    print("\n⚙️ Processing audio with official pretrained BS-RoFormer Band-Split... Please wait!")
    with torch.no_grad():
        stems = apply_model(
            model,
            audio_tensor,
    ...
    # Header for results
    print("\n" + "="*60)
    print("🎧 PRETRAINED BS-ROFORMER BAND-SPLIT SEPARATION RESULTS (BROWSER SAFE)")
    print("="*60)
    ...
    if "vocals" in stem_dict and "other" in stem_dict:
        ...
        display_audio_preview(
            target_wav,
            "bsroformer_target_vocals_plus_other.wav",
            sr=sr,
            preview_sec=PREVIEW_SECONDS,
    ...
    print("="*60)
    print(f"📦 Model Name:               Pretrained BS-RoFormer / Band-Split ({MODEL_NAME})")
...
# ==============================================================================
# 🏁 END OF SCRIPT — LIVE DEMO TEST (PRETRAINED BS-ROFORMER BAND-SPLIT)
# ==============================================================================
