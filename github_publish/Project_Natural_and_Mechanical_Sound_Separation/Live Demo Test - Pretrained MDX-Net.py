# ==============================================================================
# 🎧 LIVE DEMO TEST — PRETRAINED META MDX-NET MODEL
# 📌 Project: Natural and Mechanical Sound Separation (NatSep)
# 📄 Description: Real-time inference test with official Meta MDX-Net model.
...
# 📌 2. OFFICIAL PRETRAINED MDX-NET MODEL NAME
# Options: 'mdx_extra' (Meta's best pretrained MDX-Net model), 'mdx', 'mdx_q'
...
def main():
    ...
    # Load Official Pretrained Meta MDX-Net Model
    print(f"⏳ Downloading & Loading Official Pretrained MDX-Net Model '{MODEL_NAME}'...")
    try:
        model = demucs.pretrained.get_model(MODEL_NAME)
    except Exception as e:
        print(f"⚠️ Could not load '{MODEL_NAME}', falling back to 'mdx': {e}")
        model = demucs.pretrained.get_model("mdx")

    model.to(device)
    model.eval()
    print(f"✅ Pretrained MDX-Net Loaded! Model sources: {model.sources}")
    ...
    if audio.shape[-1] > int(PREVIEW_SECONDS * sr):
        orig_preview = f"preview_original_mdx_{os.path.basename(INPUT_AUDIO_PATH)}.wav"
    ...
    print("\n⚙️ Processing audio with official pretrained MDX-Net... Please wait!")
    with torch.no_grad():
        stems = apply_model(
            model,
            audio_tensor,
    ...
    # Header for results
    print("\n" + "="*60)
    print("🎧 PRETRAINED MDX-NET SEPARATION RESULTS (BROWSER SAFE)")
    print("="*60)
    ...
    if "vocals" in stem_dict and "other" in stem_dict:
        ...
        display_audio_preview(
            target_wav,
            "pretrained_mdx_target_vocals_plus_other.wav",
            sr=sr,
            preview_sec=PREVIEW_SECONDS,
    ...
    if "drums" in stem_dict and "bass" in stem_dict:
        ...
        display_audio_preview(
            bg_wav,
            "pretrained_mdx_removed_drums_and_bass.wav",
            sr=sr,
            preview_sec=PREVIEW_SECONDS,
...
# ==============================================================================
# 🏁 END OF SCRIPT — LIVE DEMO TEST (PRETRAINED META MDX-NET)
# ==============================================================================
