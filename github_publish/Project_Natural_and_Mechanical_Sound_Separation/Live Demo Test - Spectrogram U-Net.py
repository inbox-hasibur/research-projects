# ==============================================================================
# 🎧 LIVE DEMO TEST — PRETRAINED SPECTROGRAM U-NET MODEL (HDEMUCS MMI)
# 📌 Project: Natural and Mechanical Sound Separation (NatSep)
# 📄 Description: Real-time inference test with official Pretrained Spectrogram U-Net model.
...
# 📌 2. OFFICIAL PRETRAINED SPECTROGRAM U-NET MODEL NAME
# Options: 'hdemucs_mmi' (Meta's 84M Hybrid Conv U-Net), 'hdemucs'
...
def main():
    ...
    # Load Official Pretrained Spectrogram U-Net Model (hdemucs_mmi)
    print(f"⏳ Downloading & Loading Official Pretrained Spectrogram Conv U-Net Model '{MODEL_NAME}'...")
    try:
        model = demucs.pretrained.get_model(MODEL_NAME)
    except Exception as e:
        print(f"⚠️ Could not load '{MODEL_NAME}', falling back to 'hdemucs': {e}")
        model = demucs.pretrained.get_model("hdemucs")

    model.to(device)
    model.eval()
    print(f"✅ Official Pretrained Spectrogram U-Net Model Loaded! Model sources: {model.sources}")
    ...
    if audio.shape[-1] > int(PREVIEW_SECONDS * sr):
        orig_preview = f"preview_original_unet_{os.path.basename(INPUT_AUDIO_PATH)}.wav"
    ...
    print("\n⚙️ Processing audio with official pretrained Spectrogram U-Net (HDemucs MMI)... Please wait!")
    with torch.no_grad():
        stems = apply_model(
            model,
            audio_tensor,
    ...
    # Header for results
    print("\n" + "="*60)
    print("🎧 PRETRAINED SPECTROGRAM U-NET SEPARATION RESULTS (BROWSER SAFE)")
    print("="*60)
    ...
    if "vocals" in stem_dict and "other" in stem_dict:
        ...
        display_audio_preview(
            target_wav,
            "spectrogram_unet_target_vocals_plus_other.wav",
            sr=sr,
            preview_sec=PREVIEW_SECONDS,
    ...
    print("="*60)
    print(f"📦 Model Name:               Pretrained Spectrogram U-Net ({MODEL_NAME})")
...
# ==============================================================================
# 🏁 END OF SCRIPT — LIVE DEMO TEST (PRETRAINED SPECTROGRAM U-NET)
# ==============================================================================
