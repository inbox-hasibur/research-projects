...
# 📌 Project: Natural and Mechanical Sound Separation (NatSep V2.2)
...
# 📌 1. HYBRID U-DIT V2.2 MODEL PATH
MODEL_PATH = "/kaggle/input/models/inboxhasibur/v2-2-hybrid-u-dit/pytorch/default/1/best_hybrid_udit"
...
class UNetDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 out_channels=1):
        super().__init__()
        if conv3_in is None: conv3_in = up3_out + up3_out
        if conv2_in is None: conv2_in = up2_out + up2_out
        if conv1_in is None: conv1_in = up1_out + up1_out
...
def build_model_from_checkpoint(state_dict, device):
    ...
    dit_depth    = sum(1 for k in sd if k.startswith("dit_backbone.transformer.layers.") and k.endswith(".self_attn.in_proj_weight"))
    ...
    conv3_in  = sd["decoder.conv3a.weight"].shape[1]
    conv2_out = sd["decoder.conv2a.weight"].shape[0]
    conv2_in  = sd["decoder.conv2a.weight"].shape[1]
    conv1_in  = sd["decoder.conv1a.weight"].shape[1]
    print(f"ℹ️  Checkpoint dims: DiT channels={dit_channels}, ffn={dit_ffn}, depth={dit_depth}")
...
def main():
    ...
    print("\n⏳ Loading Hybrid U-DiT V2.2 weights & reconstructing pruned architecture...")
    model = None
    ...
    if os.path.exists(MODEL_PATH):
        ...
        if os.path.isdir(MODEL_PATH):
            ...
            zip_path = "/kaggle/working/temp_hybrid_udit_v22.pth"
        ...
        try:
            ...
            print("✅ Hybrid U-DiT V2.2 model weights loaded successfully.")
    ...
    if audio.shape[-1] > int(PREVIEW_SECONDS * sr):
        orig_preview = f"preview_original_v22_{os.path.basename(INPUT_AUDIO_PATH)}.wav"
    ...
    print("\n⚙️ Processing audio with Hybrid U-DiT V2.2... Please wait!")
    ...
    output_filename = "v2_2_separated_output_hybrid_udit.wav"
    ...
    print("="*60)
    print(f"📦 Model Name:               Hybrid U-DiT V2.2 (Edge SOTA)")
...
# ==============================================================================
# 🏁 END OF SCRIPT — LIVE DEMO TEST (HYBRID U-DIT V2.2)
# ==============================================================================
